...
void Disjoint_Set::Print() const
{
  ...
  sprintf(buf, "%d", (int) Links.size());
  fw = strlen(buf);
  if (fw == 1) fw = 2;

  printf("\n");
  printf("Node:  ");
  for (i = 0; i < Links.size(); i++) printf(" %*lu", fw, i);  
  printf("\n");

  printf("Links: ");
  for (i = 0; i < Links.size(); i++) printf(" %*d", fw, Links[i]);  
  printf("\n");
...
size_t Disjoint_Set::Size() const { return Links.size(); }
const vector <int> *Disjoint_Set::Get_Links() const { return &Links; }
const vector <int> *Disjoint_Set::Get_Ranks() const { return &Ranks; }
const vector <int> *Disjoint_Set::Get_Sizes() const { return &Sizes; }
const vector <int> *Disjoint_Set::Get_Set_Ids() const { return &Set_Ids; }
const vector <int> *Disjoint_Set::Get_Set_Id_Indices() const { return &Set_Id_Indices; }
const vector < list<int> > *Disjoint_Set::Get_Elements() const { return &Elements; }
